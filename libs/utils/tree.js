 ...
 * @param {any} coll
 * @param {any} childkey
 * @param {any} pre  先序访问的 callback, 必须要有返回值
 * @param {any} post  后续访问的 callback, 必须要有返回值
 * @param {any} [initialValue={}] pre 和 post 的第二个参数
 * @returns
 */
...
/**
 * mutable: it return a new tree like object
 *
 * @export
 * @param {Array} coll
 * @param {String} childkey
 ...
 * @returns
 */
export function filterTreeMutable(coll, childkey, callback) {
  if (!coll || coll === undefined || coll.length === 0) {
    return coll
  }
  // console.log(coll)
  coll = coll.filter(callback).map(item => {
    // console.log(item)
    if (item[childkey]) {
      item[childkey] = filterTreeMutable(item[childkey], childkey, callback)
      // 如果当前节点不满足，但是孩子节点可以满足的，那么僵当前节点替换为孩子节点
      // 所以需要先遍历到叶子节点，再向上回溯
      // if (!callback(item)) {
      //   ret = ret[childkey]
      // }
      if (item[childkey].length < 1) {
        delete item[childkey]
      }
    }
